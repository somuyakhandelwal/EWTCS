...
### Kiosk Mode Sessions (Nurse Workstations)
- **Binding**: Kiosk sessions are cryptographically bound to the client IP address. IP mismatches trigger immediate revocation.
- **Persistence**: Sessions ignore the standard 30-minute inactivity timeout to support continuous ward operation.
- **Restoration**: Upon browser restart, the system automatically restores sessions using a secure recovery token.
- **Logging**: Automatic restorations are logged as `kiosk_session_restored` events.
- **Revocation**: Admins can force-logout any kiosk station via the Admin Panel.
