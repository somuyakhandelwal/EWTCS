...
export async function getPatientStats(
  ...
  const result = await query<RawPatientStats>(
    `
    SELECT
...
export async function getAvgStageTime(dayStart: Date, dayEnd: Date): Promise<number> {
  const result = await query<RawAvgStageTime>(
    `
...
export async function getDelayCount(dayStart: Date, dayEnd: Date): Promise<number> {
  const result = await query<RawDelayCount>(
    `
...
export async function getMostDelayedStage(
  dayStart: Date,
  dayEnd: Date
): Promise<string | undefined> {
  const result = await query<RawMostDelayedStage>(
    `
