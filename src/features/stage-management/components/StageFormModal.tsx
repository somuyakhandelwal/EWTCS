...
import {
  ...
  getSupportedStageColors,
} from "@/shared/utils/stage-colors";
...
export function StageFormModal({
  ...
  const [color, setColor] = useState(
    normalizeStageColor(stage?.color_code) ?? "blue",
  );
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <label className="block text-sm font-semibold text-gray-700 mb-1">
          Stage Name{" "}
          <span className="text-gray-400 font-normal">({name.length}/50)</span>
        </label>
        <input
          value={name}
          ...
          {COLORS.map((c) => {
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center w-10 h-10 rounded-full border-2 ${colorClasses.bg} ${colorClasses.border} ${
                  color === c
