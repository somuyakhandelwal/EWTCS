...
export function BedDashboardClient({ initialData }: BedDashboardClientProps) {
  ...
  const { data: realtimeData, connectionStatus, isLoading, refresh, reconnect } = useRealtimeBedUpdates(initialData)
  
  // Local state for optimistic updates during stage changes
  const [data, setData] = useState<BedGridData>(realtimeData)
