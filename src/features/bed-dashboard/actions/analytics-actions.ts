...
export async function fetchLongestWaitingBeds(limit: number = 10): Promise<{
  ...
  try {
    ...
    logger.info('Fetched longest waiting beds', { userId: session.userId, count: beds.length, limit })
...
export async function exportStageTransitionsAsCSV(options?: {
  ...
  try {
    const session = await requireRole(['supervisor', 'admin'])
    const transitions = await getStageTransitions(options?.startDate, options?.endDate, options?.bedId, options?.stageId)
    const headers = ['ID', 'Bed Number', 'From Stage', 'To Stage', 'Transition Time', 'Duration Prev (ms)', 'Duration Curr (ms)', 'Changed By', 'Notes']
    const rows = transitions.map((t: StageTransitionRecord) => [
      t.id, t.bedNumber, t.fromStageName || 'N/A', t.toStageName,
      t.transitionTime.toISOString(),
      t.durationInPreviousStageMs?.toString() || 'N/A',
      t.durationInCurrentStageMs?.toString() || 'N/A',
      t.changedByUsername, t.notes || '',
    ])
    const csv = [headers, ...rows].map((row) => row.map((cell: string) => `"${cell}"`).join(',')).join('\n')
    logger.info('Exported stage transitions as CSV', { userId: session.userId, count: transitions.length })
