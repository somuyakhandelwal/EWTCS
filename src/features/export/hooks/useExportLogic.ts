...
import type { ExportScope, ExportFormat, ResolvedDateRange, PdfSection } from '../types/export.types'
import {
...
export function useExportLogic({
    ...
    const handleCsvExport = useCallback(
        async (r: ResolvedDateRange) => {
            ...
            downloadCsv(result.data, buildFilename(scope, r, 'csv'))
            onClose()
        },
        [scope, shiftId, onClose]
    )
    ...
    const handlePdfExport = useCallback(
        ...
        [scope, pdfSections, pdfTitle, exportedBy, onClose]
    )
    ...
    return {
        ...
        setRange,
        error,
        isLoading: isPending || pdfProgress !== null,
        pdfProgress,
        handleExport,
    }
}
