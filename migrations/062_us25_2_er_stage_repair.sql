...
-- Up Migration
-- ============================================================
-- Step 1: Deactivate legacy ER stages
...
VALUES
  ...
  -- Empty → Initial Investigation: patient arrives and begins workup
  (
    (SELECT id FROM stages WHERE name = 'Empty'),
    (SELECT id FROM stages WHERE name = 'Initial Investigation'),
    true, false,
    'Patient admitted. Doctor begins initial assessment and investigation.',
    10
  ),
  -- Initial Investigation → Initial Treatment: assessment complete, treatment starts
  (
    (SELECT id FROM stages WHERE name = 'Initial Investigation'),
    (SELECT id FROM stages WHERE name = 'Initial Treatment'),
    true, false,
    'Initial investigation complete. First-line treatment commenced.',
    10
  ),
  -- Initial Treatment → Drugs/Test: awaiting medications or test results
  (
    (SELECT id FROM stages WHERE name = 'Initial Treatment'),
    (SELECT id FROM stages WHERE name = 'Drugs/Test'),
    true, false,
    'Treatment administered. Awaiting drug response or diagnostic results.',
    10
  ),
  -- Drugs/Test → Observation: results in, patient monitored
  (
    (SELECT id FROM stages WHERE name = 'Drugs/Test'),
    (SELECT id FROM stages WHERE name = 'Observation'),
    true, false,
    'Test results received. Patient placed under observation.',
    10
  ),
  -- Observation → Decision Made: outcome clear, disposition decided
  (
    (SELECT id FROM stages WHERE name = 'Observation'),
    (SELECT id FROM stages WHERE name = 'Decision Made'),
    true, false,
    'Observation complete. Clinical decision made for discharge or admission.',
    10
  ),
  -- Decision Made → Discharge Process: discharge initiated
  (
    (SELECT id FROM stages WHERE name = 'Decision Made'),
    (SELECT id FROM stages WHERE name = 'Discharge Process'),
    true, false,
    'Decision documented. Patient being discharged or transferred.',
    10
  ),
  -- Discharge Process → Cleaning: patient has left, bed cleaned
  (
    (SELECT id FROM stages WHERE name = 'Discharge Process'),
    (SELECT id FROM stages WHERE name = 'Cleaning'),
    true, false,
    'Patient discharged. Bed cleaning and preparation started.',
    10
  ),
  -- Cleaning → Empty: bed ready for next patient
  (
    (SELECT id FROM stages WHERE name = 'Cleaning'),
    (SELECT id FROM stages WHERE name = 'Empty'),
    true, false,
    'Bed cleaned and prepared. Ready for next patient.',
    10
  ),
  -- ── BACKWARD TRANSITIONS (require supervisor override) ────────────────────
  -- Observation → Initial Treatment: condition worsened, treatment restarted
  (
    (SELECT id FROM stages WHERE name = 'Observation'),
    (SELECT id FROM stages WHERE name = 'Initial Treatment'),
    false, true,
    'Patient condition deteriorated during observation. Return to treatment.',
    9
  ),
  -- Drugs/Test → Initial Investigation: test results require reassessment
  (
    (SELECT id FROM stages WHERE name = 'Drugs/Test'),
    (SELECT id FROM stages WHERE name = 'Initial Investigation'),
    false, true,
    'Test results indicate need for reassessment. Doctor to review.',
    9
  ),
  -- Initial Treatment → Initial Investigation: reassessment needed
  (
    (SELECT id FROM stages WHERE name = 'Initial Treatment'),
    (SELECT id FROM stages WHERE name = 'Initial Investigation'),
    false, true,
    'Clinical picture changed during treatment. Reassessment required.',
    9
  ),
  -- ── SKIP TRANSITIONS (require supervisor override) ────────────────────────
  -- Initial Investigation → Decision Made: minor case, immediate discharge
  (
    (SELECT id FROM stages WHERE name = 'Initial Investigation'),
    (SELECT id FROM stages WHERE name = 'Decision Made'),
    false, true,
    'Minor presentation. Patient cleared for discharge without full treatment pathway.',
    8
  ),
  -- Empty → Observation: emergency readmission or critical patient
  (
    (SELECT id FROM stages WHERE name = 'Empty'),
    (SELECT id FROM stages WHERE name = 'Observation'),
    false, true,
    'Emergency readmission. Patient placed directly under observation.',
    8
  ),
  -- Empty → Initial Treatment: immediate treatment required
  (
    (SELECT id FROM stages WHERE name = 'Empty'),
    (SELECT id FROM stages WHERE name = 'Initial Treatment'),
    false, true,
    'Critical case. Immediate treatment started on arrival.',
    8
  )
