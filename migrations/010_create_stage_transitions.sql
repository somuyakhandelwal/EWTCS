...
-- Expected workflow: Empty → Registration → Doctor Assessment →
--                    Treatment/Observation → Decision Made → Discharge Process → Cleaning → Empty
...
VALUES
    ...
    -- (Triage removed from active ER workflow; emergency re-triage handled in intake service)
    
    
    -- SKIP TRANSITIONS (usually blocked, very special cases)
    
    -- (Skip transitions involving triage have been removed; emergencies handled via intake flows)
