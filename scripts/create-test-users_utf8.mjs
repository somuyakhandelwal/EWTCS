...
async function createTestUsers() {
    ...
    try {
        ...
        console.log('Γ£ô Connected to database');
        ...
        if (existingUsers.rows.length > 0) {
            console.log('\nΓÜá∩╕Å  Test users already exist:');
        ...
        for (const user of testUsers) {
            ...
            console.log(`Γ£ô Created: ${user.username} (${user.role})`);
            console.log(`  Password: ${user.password}`);
        }

        console.log('\nΓ£à All test users created successfully!\n');
        console.log('≡ƒôï Login credentials:');
